    ...
    <div className="min-h-screen flex flex-col items-center justify-start p-4 md:p-12 lg:p-20 pt-10 md:pt-20">
      <div className="max-w-6xl w-full space-y-8 md:space-y-12">
        <div className="space-y-3 md:space-y-4 text-center">
          <Badge variant="outline" className="px-3 md:px-6 py-1 border-primary/20 text-primary font-bold uppercase tracking-[0.2em] md:tracking-[0.4em] bg-white/50 backdrop-blur-sm shadow-sm text-[8px] md:text-[10px]">
            MUN Operating System
          </Badge>
          <h1 className="text-4xl md:text-7xl lg:text-8xl font-black tracking-tighter text-gradient uppercase font-headline leading-none">
            MUN-OS
          </h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8">
          {isLoading ? (
            Array(3).fill(0).map((_, i) => (
              <Card key={i} className="animate-pulse bg-white/50 border-none h-[160px] md:h-[220px] rounded-2xl md:rounded-3xl" />
            ))
          ) : (
            committees?.map((committee) => (
              <Card key={committee.id} className="glass-card group hover:scale-[1.02] transition-all duration-300 rounded-[1.25rem] md:rounded-3xl overflow-hidden">
                <CardHeader className="bg-gradient-to-b from-primary/5 to-transparent pb-3 pt-4 md:pt-6">
                  <div className="flex justify-between items-start mb-2 md:mb-3">
                    ...
                    <Badge className="bg-primary/10 text-primary border-primary/20 uppercase text-[7px] md:text-[9px] font-bold px-2 py-0.5">
                      {committee.language === 'en' ? 'Active' : 'Actif'}
                    </Badge>
                  </div>
                  <CardTitle className="text-base md:text-xl font-black uppercase tracking-tight text-foreground/90">{committee.name}</CardTitle>
                </CardHeader>
                <CardContent className="pt-1 md:pt-2 space-y-2 md:space-y-3">
                  <div className="grid grid-cols-1 gap-2 md:gap-2.5">
                    <Button asChild className="w-full bg-primary hover:bg-primary/90 rounded-xl h-10 md:h-12 text-[10px] md:text-xs font-bold shadow-lg shadow-primary/20 group">
                    ...
                    <Button asChild variant="outline" className="w-full border-primary/20 text-primary font-bold rounded-xl h-10 md:h-12 text-[10px] md:text-xs hover:bg-primary/5">
          ...
          {committees?.length === 0 && !isLoading && (
            <Card className="col-span-full py-10 md:py-20 border-dashed border-2 bg-white/30 rounded-2xl md:rounded-3xl flex flex-col items-center justify-center text-muted-foreground">
              <PlusCircle className="size-8 md:size-10 mb-3 opacity-10" />
              <p className="italic text-xs md:text-base font-medium opacity-50 text-center px-6">En attente de la création des comités par l'administration.</p>
            </Card>
          )}
        </div>
      ...
      <footer className="mt-4 md:mt-8 py-4 mb-4">
        <a 
