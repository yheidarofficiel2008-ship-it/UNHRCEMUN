...
import { ShieldAlert, Plus, Trash2, Key, Home, Globe, Languages, LogIn, User } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen p-4 md:p-12 lg:p-20">
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-16">
        <header className="flex flex-col md:flex-row justify-between items-center md:items-end gap-4 md:gap-8 text-center md:text-left">
          <div className="space-y-2 md:space-y-4">
            <Badge variant="outline" className="font-black uppercase tracking-widest px-3 md:px-4 py-1 border-primary/20 text-primary bg-white/50 backdrop-blur-sm text-[7px] md:text-xs">Admin Panel</Badge>
            <h1 className="text-3xl md:text-7xl lg:text-8xl font-black uppercase tracking-tighter text-gradient leading-none">Management</h1>
          </div>
          <Button asChild variant="outline" size="lg" className="font-black uppercase tracking-widest text-[8px] md:text-[10px] h-10 md:h-14 rounded-xl md:rounded-2xl px-5 md:px-8 border-primary/10 hover:bg-primary/5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-12 items-start">
          <div className="lg:col-span-4">
            <Card className="glass-card rounded-[1.25rem] md:rounded-[2.5rem] overflow-hidden border-primary/10 shadow-2xl">
              <CardHeader className="bg-primary/[0.03] border-b border-primary/5 p-5 md:p-8">
                <CardTitle className="flex items-center gap-2 md:gap-3 text-lg md:text-2xl font-black uppercase tracking-tight text-gradient"><Plus className="text-primary size-5 md:size-6" /> Nouveau Comité</CardTitle>
                <CardDescription className="text-[9px] md:text-sm font-medium text-muted-foreground/80">Espace de travail.</CardDescription>
              </CardHeader>
              <form onSubmit={handleCreate}>
                <CardContent className="space-y-4 md:space-y-6 p-5 md:p-8">
                  <div className="space-y-1">
                    <Label className="text-[8px] md:text-[10px] uppercase font-black tracking-widest text-primary/60">Nom Officiel</Label>
                  ...
                  <div className="space-y-1">
                    <Label className="text-[8px] md:text-[10px] uppercase font-black tracking-widest text-primary/60">Langue</Label>
                    <Select value={newCommittee.language} onValueChange={(val) => setNewCommittee({...newCommittee, language: val})}>
                      <SelectTrigger className="rounded-xl h-10 md:h-12 border-primary/10 text-[9px] md:text-xs">
                        <div className="flex items-center gap-2"><Languages className="size-3.5 md:size-4 text-primary/60" /><SelectValue /></div>
                      </SelectTrigger>
                  ...
                  <div className="space-y-1">
                    <Label className="text-[8px] md:text-[10px] uppercase font-black tracking-widest text-primary/60">Email</Label>
                  ...
                  <div className="space-y-1">
                    <Label className="text-[8px] md:text-[10px] uppercase font-black tracking-widest text-primary/60">Code d'Accès</Label>
          ...
          <div className="lg:col-span-8 space-y-6 md:space-y-8">
            <h2 className="text-[8px] md:text-xs font-black uppercase tracking-[0.2em] md:tracking-[0.4em] text-primary/40 flex items-center gap-3 md:gap-4 before:h-px before:flex-1 before:bg-primary/10 after:h-px after:flex-1 after:bg-primary/10 whitespace-nowrap">
              <Globe className="size-3.5 md:size-4" /> Comités ({committees?.length || 0})
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  ...
                  <CardContent className="p-4 md:p-8 flex justify-between items-start gap-4">
                    <div className="space-y-3 md:space-y-4 min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-base md:text-2xl font-black uppercase tracking-tight text-gradient group-hover:text-primary transition-colors truncate">{c.name}</h3>
                        <Badge variant="outline" className="uppercase text-[6px] md:text-[8px] font-black tracking-widest border-primary/10 text-primary/60 bg-primary/5 px-1.5">{c.language || 'fr'}</Badge>
                      </div>
                      <div className="space-y-1 opacity-60">
                        ...
                        <p className="text-[7px] md:text-[9px] font-mono uppercase tracking-tighter bg-muted px-1.5 py-0.5 rounded-lg w-fit">ID: {c.id}</p>
                      </div>
                    </div>
                    
                    <AlertDialog onOpenChange={(open) => !open && setDeleteVerificationKey('')}>
                      <AlertDialogTrigger asChild>
                        <Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/5 rounded-xl md:rounded-2xl size-8 md:size-12 border border-transparent hover:border-destructive/10 shrink-0">
                          <Trash2 className="size-4 md:size-6" />
                        </Button>
                      </AlertDialogTrigger>
                      <AlertDialogContent className="rounded-[1.25rem] md:rounded-[2.5rem] border-destructive/10 p-5 md:p-10 w-[95vw] max-w-lg">
                        <AlertDialogHeader className="space-y-1.5 md:space-y-4">
                          <AlertDialogTitle className="text-lg md:text-3xl font-black uppercase tracking-tight text-destructive">Suppression Critique</AlertDialogTitle>
                          <AlertDialogDescription className="text-[9px] md:text-base font-medium leading-relaxed">
                            Suppression définitive de <span className="text-foreground font-black">"{c.name}"</span>.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <div className="py-3 md:py-8 space-y-1.5 md:space-y-3">
                          <Label htmlFor="delete-key" className="text-[8px] md:text-[10px] uppercase font-black tracking-widest text-destructive/60 ml-1">Confirmation</Label>
                          <Input 
                            id="delete-key"
                            type="password" 
                            placeholder="..."
                            ...
                            className="font-mono h-11 md:h-14 rounded-xl md:rounded-2xl border-destructive/10 text-base md:text-xl tracking-[0.2em] md:tracking-[0.3em] text-center"
                          />
                        </div>
                        <AlertDialogFooter className="flex flex-col md:flex-row gap-2 md:gap-4">
                          <AlertDialogCancel className="rounded-xl h-10 md:h-12 border-primary/10 font-bold text-[9px] md:text-xs" onClick={() => setDeleteVerificationKey('')}>Annuler</AlertDialogCancel>
                          <AlertDialogAction 
                            onClick={() => {
                              handleDelete(c.id);
              ...
              {committees?.length === 0 && (
                <div className="col-span-full py-12 md:py-40 border-2 md:border-4 border-dashed rounded-[1.25rem] md:rounded-[3rem] border-primary/5 flex flex-col items-center gap-3 md:gap-6 opacity-10">
                  <Globe className="size-8 md:size-20" />
                  <p className="text-base md:text-2xl font-black uppercase tracking-widest">Aucune session</p>
                </div>
              )}
            </div>
          </div>
