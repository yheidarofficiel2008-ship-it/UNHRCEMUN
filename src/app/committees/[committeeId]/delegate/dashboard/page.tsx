...
import { Send, CheckCircle2, XCircle, LogOut, FileText, Monitor, Clock, Timer, MessageSquarePlus, MessageSquare, Check, Bold, Italic, Underline, Eye, ThumbsUp, ThumbsDown, CircleSlash, ShieldAlert, AlertTriangle, User, Users } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex flex-col relative">
      ...
      {isCountrySuspended && (
        <div className="fixed inset-0 z-[10000] bg-destructive/95 flex flex-col items-center justify-center p-8 text-white animate-in fade-in zoom-in duration-300">
          ...
          <p className="text-2xl text-center opacity-90 max-w-2xl">Votre délégation a été suspendue. Veuillez patienter.</p>
        </div>
      )}
          ...
          <div className="max-w-4xl w-full text-center space-y-8">
            ...
            {activeOverlay.type === 'vote' && (
              <div className="space-y-10">
                ...
                {hasVoted && <div className="text-lg md:text-xl font-bold uppercase tracking-widest text-white/80 animate-pulse bg-white/20 py-4 rounded-full">Vote enregistré</div>}
              </div>
            )}
          </div>
        </div>
      ...
      <main className="flex-1 p-6 grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-[1400px] mx-auto w-full">
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card>
            <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-lg"><Send size={18} /> Mes Envois</CardTitle></CardHeader>
                  ...
                  {myEnvois.map(item => (
                    <div key={item.id} className="p-3 border rounded-lg bg-muted/10 text-xs">
                      <div className="flex justify-between items-center mb-2">
                        <Badge variant="outline" className="text-[10px] uppercase">{item._type === 'resolution' ? 'Résolution' : item.type === 'privilege' ? 'Privilège' : 'Message'}</Badge>
        ...
        <div className="lg:col-span-8 space-y-6">
          <Card className="border-secondary/20 shadow-xl overflow-hidden">
            <CardHeader className="bg-secondary/5 border-b pb-4">
              <div className="flex justify-between items-center"><Badge className="bg-secondary">SÉANCE EN COURS</Badge></div>
              <CardTitle className="text-3xl font-headline mt-2 break-words">{isActive ? currentAction.title : 'En attente...'}</CardTitle>
            </CardHeader>
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    ...
                    <div className="bg-muted/30 p-8 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-4">
                      <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground uppercase tracking-widest"><Timer size={18} /> Chrono Orateur</div>
              ...
              <CardHeader className="border-b border-primary/20 pb-4 bg-white/50">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <Badge className="bg-primary mb-2"><Monitor size={12} className="mr-1" /> PROJETÉ AU COMITÉ</Badge>
          ...
          <Card className={`shadow-xl ${(!allowResolutions || isCountrySuspended || activeOverlay?.type === 'crisis') ? 'opacity-70 grayscale pointer-events-none' : ''}`}>
            ...
            <form onSubmit={submitResolution}>
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input placeholder="Titre de la résolution" value={resolutionForm.title} onChange={e => setResolutionForm({...resolutionForm, title: e.target.value})} required disabled={!allowResolutions} />
                  </div>
                  <div className="space-y-2">
                    <Label className="font-bold">Porte-parole</Label>
                    <Input placeholder="Nom du pays" value={resolutionForm.spokesperson} onChange={e => setResolutionForm({...resolutionForm, spokesperson: e.target.value})} required disabled={!allowResolutions} />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="font-bold">Pays Sponsors</Label>
                  <Input placeholder="France, Japon, Brésil..." value={resolutionForm.sponsors} onChange={e => setResolutionForm({...resolutionForm, sponsors: e.target.value})} disabled={!allowResolutions} />
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-center"><Label className="font-bold">Texte</Label>
              ...
              <CardFooter><Button type="submit" disabled={!allowResolutions} className="w-full h-16 bg-secondary text-xl font-bold gap-3 shadow-lg"><Send size={24} /> Transmettre au Bureau</Button></CardFooter>
            </form>
          </Card>
        </div>
      </main>
